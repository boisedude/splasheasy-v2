import { CheckCircle, AlertTriangle, XCircle, Info, TrendingUp, TrendingDown, Minus, Cloud } from 'lucide-react'
...
function getStatus(value: number | null, ideal: { min: number; max: number }, acceptable: { min: number; max: number }) {
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="mb-3">
        ...
        <div className="text-sm text-gray-500 flex items-center justify-between">
          <span>Ideal: {chemical.ideal.min}-{chemical.ideal.max} {chemical.unit}</span>
...
export default function TestResults({ 
      ...
      <div className={`max-w-2xl mx-auto bg-white rounded-lg shadow-lg ${className}`}>
        <div className="p-6 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
            Unable to Read Test Strip
          </h2>
          <p className="text-gray-600 mb-6">
            The test strip image was not clear enough for accurate analysis. 
            Please try again with better lighting and a clearer image.
          </p>
  ...
  const overallStatus = chemicals.some(c => getStatus(c.value, c.ideal, c.acceptable) === 'needs-attention')
    ? 'needs-attention'
  ...
  return (
    <div className={`max-w-4xl mx-auto bg-gray-50 rounded-lg shadow-lg ${className}`}>
      {/* Header */}
      <div className="bg-white rounded-t-lg border-b border-gray-200 p-6">
            ...
            <div className="flex items-center space-x-4 mb-2">
              ...
              {historicalContext && (
                <span className={`text-xs px-2 py-1 rounded-full ${
                  historicalContext.overall_pool_health === 'excellent' ? 'bg-green-100 text-green-800' :
                  historicalContext.overall_pool_health === 'good' ? 'bg-blue-100 text-blue-800' :
                  historicalContext.overall_pool_health === 'needs_attention' ? 'bg-yellow-100 text-yellow-800' :
        ...
        {results.notes && (
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <h4 className="font-medium text-blue-900 mb-1">AI Analysis Notes</h4>
          ...
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              ...
              {weatherRecommendations.weather_notes && (
                <p className="text-yellow-700 text-xs">{weatherRecommendations.weather_notes}</p>
              )}
            </div>
          </div>
        )}
        ...
        {historicalContext && historicalContext.maintenance_insights.length > 0 && (
          <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
            <h4 className="font-medium text-purple-900 mb-2">Historical Insights</h4>
            <div className="space-y-1">
              {historicalContext.maintenance_insights.map((insight, index) => (
                <p key={index} className="text-purple-700 text-sm">• {insight}</p>
              ))}
              <p className="text-purple-800 text-sm font-medium mt-2">
      ...
      <div className="p-6">
        <div className="grid md:grid-cols-2 gap-4 mb-6">
          {chemicals.map((chemical) => {
            const paramKey = chemical.name.toLowerCase().replace(/\s+/g, '_').replace('level', '').trim() as keyof typeof results.individual_confidence
            const individualConfidence = results.individual_confidence?.[paramKey === 'ph' ? 'ph' : paramKey === 'free_chlorine' ? 'free_chlorine' : paramKey === 'total_alkalinity' ? 'total_alkalinity' : paramKey === 'cyanuric_acid' ? 'cyanuric_acid' : 'total_hardness']
            const trend = historicalContext?.trends.find(t => 
